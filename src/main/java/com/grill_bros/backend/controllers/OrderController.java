...
public class OrderController {
    ...
    @PostMapping
    @Operation(
            summary     = "Create a new order from cart contents",
            description = "Submits the customer's cart at checkout. Cart is managed on the " +
                    "frontend; this endpoint receives the final resolved list of items. " +
                    "Returns the created order including its UUID (needed for payment) " +
    ...
    @ApiResponses({
            ...
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "404", description = "One or more menu items not found"),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "422", description = "One or more items currently unavailable")
    })
    public ResponseEntity<ApiResponse<OrderResponse>> createOrder(
            @Valid @RequestBody
            @io.swagger.v3.oas.annotations.parameters.RequestBody(
                    description = "Customer details and cart items",
                    content = @Content(
        ...
        log.info("Checkout request received for phone={} items={}",
                req.getCustomerPhone(), req.getItems().size());
    ...
    @Operation(
            ...
            description = "Returns the current status and full line items for an order. " +
                    "Order numbers follow the format QB-YYYYMMDD-XXXX. " +
                    "Use this to build a customer-facing order tracking page."
    )
    @ApiResponses({
    ...
    public ResponseEntity<ApiResponse<OrderResponse>> getOrder(
            @Parameter(
                    description = "Human-readable order number (e.g. QB-20250601-0042)",
                    example     = "QB-20250601-0042",
                    required    = true
            )
        ...
        return ResponseEntity.ok(
                ApiResponse.ok(orderService.getByOrderNumber(orderNumber)));
    }
}
